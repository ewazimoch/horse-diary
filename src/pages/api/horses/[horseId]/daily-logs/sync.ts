...
import { syncDailyLogsEnvelopeSchema } from "@/lib/validation/daily-logs.schema";
import { syncDailyLogs } from "@/lib/services/daily-logs.service";
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  const parseResult = syncDailyLogsEnvelopeSchema.safeParse(body);
  if (!parseResult.success) {
    return new Response(JSON.stringify({ error: parseResult.error.flatten() }), { status: 400 });
  }

  try {
    const result = await syncDailyLogs(locals.supabase, horseId, parseResult.data.entries);
